      ...
      <tbody className="divide-y divide-slate-100">
        {bonos.map((b, i) => (
          <tr key={b.ticker} className={ROW_CLASS} style={rowDelay(i)}>
            ...
            <td className="px-5 py-3 text-right text-base font-semibold text-slate-900">USD {b.precio.toFixed(2)}</td>
      ...
      <tbody className="divide-y divide-slate-100">
        {bonos.map((b, i) => (
          <tr key={b.ticker} className={ROW_CLASS} style={rowDelay(i)}>
            ...
            <td className="px-5 py-3 text-right text-base font-semibold text-slate-900">{formatArs(b.precioArs)}</td>
            <td className="px-5 py-3 text-right">
      ...
      <tbody className="divide-y divide-slate-100">
        {letras.map((l, i) => (
          <tr key={l.ticker} className={ROW_CLASS} style={rowDelay(i)}>
            ...
            <td className="px-5 py-3 text-slate-500">ARG</td>
            <td className="px-5 py-3 text-right text-base font-semibold text-slate-900">{formatArs(l.precioActual)}</td>
            <td className="px-5 py-3 text-right">
      ...
      <tbody className="divide-y divide-slate-100">
        {bonos.map((b, i) => (
          <tr key={b.ticker} className={ROW_CLASS} style={rowDelay(i)}>
            <td className={TICKER_CLASS}>{b.ticker}</td>
            <td className="px-5 py-3 text-slate-500">ARG</td>
            <td className="px-5 py-3 text-right text-base font-semibold text-slate-900">{formatArs(b.precio)}</td>
            <td className="px-5 py-3 text-right">
              <VarBadge value={b.variacionPorcentaje} />
            </td>
            <td className="px-5 py-3 text-right text-slate-500">{formatDate(b.vencimiento)}</td>
